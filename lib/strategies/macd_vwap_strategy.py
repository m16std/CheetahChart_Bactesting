...
class MACDVWAPStrategy(BaseStrategy):
    def __init__(self):
        super().__init__()
        self.name = "MACD VWAP Strategy"
        self.description = "Strategy based on MACD and VWAP indicators"
    ...
    def run(self, df, initial_balance, position_size, position_type, profit_factor):
        ...
        self.manager.indicators = ['macd', 'macd_signal', 'vwap']
        ...
        for i in range(len(df)):
            ...
            if not position_open:
                if (df['close'].iloc[i] > df['vwap'].iloc[i]) and df['macd'].iloc[i-1] < df['macd_signal'].iloc[i-1] and df['macd'].iloc[i] > df['macd_signal'].iloc[i]:
                    tpTriggerPx, slTriggerPx = self.manager.get_tp_sl(df, i, df['close'].iloc[i], profit_factor, 'long', 15)
                    posId = self.manager.open_position('long', 'market', tpTriggerPx, slTriggerPx, df['close'].iloc[i], qty, df.index[i])
                    position_open = True
                if (df['close'].iloc[i] < df['vwap'].iloc[i]) and df['macd'].iloc[i-1] > df['macd_signal'].iloc[i-1] and df['macd'].iloc[i] < df['macd_signal'].iloc[i]:
                    tpTriggerPx, slTriggerPx = self.manager.get_tp_sl(df, i, df['close'].iloc[i], profit_factor, 'short', 15)
