import ta
def rsi_strategy_imp(self, df, initial_balance, qty, position_type, profit_factor, leverage, commission):
    ...
    for i in range(len(df)):
        ...
        if not position_open:
            if df['rsi'].iloc[i-1] < 30 and df['rsi'].iloc[i] >= 30:              
                ...
                posId = self.open_position('long', 'market', tpTriggerPx, slTriggerPx, df['close'].iloc[i], qty, df.index[i], leverage=1)
                position_open = True
            if df['rsi'].iloc[i-1] > 70 and df['rsi'].iloc[i] <= 70:
                ...
                posId = self.open_position('short', 'market', tpTriggerPx, slTriggerPx, df['close'].iloc[i], qty, df.index[i], leverage=1)
                position_open = True
